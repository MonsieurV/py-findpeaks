...
class OctaveFindpeaksTestCase(unittest.TestCase):
    """ Test the find_peaks_cwt function from the signal processing
    module of Scipy.
    """
    ...
    def test_octave_findpeaks_equal_matlab_findpeaks_minpeakheight_1(self):
        ...
        # 'MinPeakHeight', 22
        loc = scipy.signal.find_peaks_cwt(vector, np.arange(1, 2))
        print(loc)
        self.assertEqual(
            loc,
            [8-1])
    ...
    def test_octave_findpeaks_equal_matlab_findpeaks_minpeakheight_3(self):
        ...
        # 'MinPeakHeight', 22
        loc = scipy.signal.find_peaks_cwt(vector, np.arange(2, 3))
        print(loc)
        self.assertEqual(
            loc,
            [9-1])
